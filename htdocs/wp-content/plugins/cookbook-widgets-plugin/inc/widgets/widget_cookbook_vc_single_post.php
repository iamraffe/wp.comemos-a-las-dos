...
/**************************************
WIDGET: cookbook_vc_single_post
***************************************/

	add_action('widgets_init', 'register_widget_cookbook_vc_single_post' );
	function register_widget_cookbook_vc_single_post () {
		register_widget('cookbook_vc_single_post');	
	}

	class cookbook_vc_single_post extends WP_Widget {
		...
		***************************************/
		function __construct () {
				...
				$widget_ops = array(
					'classname' => 'cookbook_vc_single_post', 								
					'description' => __('Displays a single post. Custom element for use with Visual Composer. May also be used as a widget. Please notice that this widget was designed with a specific layout in mind and may not look good in all possible layouts.', "loc_cookbook_widgets_plugin")	 				
				);
				$control_ops = array(
					'width' => 300, 
					'height' => 350, 
					'id_base' => 'cookbook_vc_single_post' 														
				);

				parent::__construct('cookbook_vc_single_post', __('Cookbook VC: Single Post', "loc_cookbook_widgets_plugin"), $widget_ops, $control_ops );	
		}
		...
		function form($instance) {
			...
			<!-- CHECKBOX -->	
				<p>
					...
					<label for="<?php echo esc_attr($this->get_field_id( 'hide_meta' )); ?>"><?php _e("Hide meta", "loc_cookbook_widgets_plugin"); ?></label>
				</p>
		...
		***************************************/
		function widget($args, $instance) {
			...
			if (!isset($widget_id)) { $widget_id = "cookbook_vc_single_post-" . uniqid(); }
													...
													<?php
														...
														if ($hide_meta != "checked") {
															...
															if ($meta == "date") {
																...
																// META DATE
								                                $archive_year  = get_the_time('Y', $this_post->ID, $this_post->ID); 
